...
class Login
{
    ...
    function setLogin(){
        ...
        $select = DB\Mysql::select(
            "SELECT * FROM adm WHERE email = :email LIMIT 1",
            [
               'email' => $this->email
            ]
        );
    ...
    {
        $select = DB\Mysql::select(
            "SELECT id FROM adm WHERE email = :email",
            [
                'email' => $this->email
            ]
        );
        ...
        if(is_array($select)){
            ...
            $insert = DB\Mysql::insert(
                "INSERT INTO adm (nome, sobrenome, email, morada, nacionalidade, genero, senha, registo, cashe, checkCashe) VALUES (:nome, :sobrenome, :email, :morada, :nacionalidade, :genero, :senha, :registo, :cashe, :checkCashe)",
                [
                    'nome'          =>  $this->nome,
    ...
    function emailCheck(){
        $select = DB\Mysql::select(
            "SELECT email FROM adm WHERE email = :email",
            [
                'email' => $this->email
            ]
        );
    ...
    {
        $select = DB\Mysql::select(
            "SELECT id, nome, sobrenome, email, cashe, checkCashe FROM adm WHERE email = :email LIMIT 1",
            [
                'email' => $this->email
            ]
        );
    ...
    {
        $select = DB\Mysql::select(
            "SELECT email, cashe FROM adm WHERE email = :email AND cashe = :cashe LIMIT 1",
            [
                'email' => $this->email,
                'cashe' => $this->cashe
            ]
